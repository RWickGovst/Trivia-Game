...
function run() {
    ...
    for (var i = 0; i < trivia[currentQ].answers.length; i++) {
        $("#quiz-area").append("<button class= 'answer-button test' id='button' data-name ='" + trivia[currentQ].answers[i] + "'>" + trivia[currentQ].answers[i] + "</button>");
    }
}
function decrement() {
    number--;
    ...
    // $("#answers").text(trivia[0].answers[0]);
    // $("#answersTwo").text(trivia[0].answers[1]);
    // $("#answersThree").text(trivia[0].answers[2]);
    ...
    $(".answer-button").on("click", function () {
        ...
        if ($(this).attr("data-name") === trivia[currentQ].correctAnswer) {
            correctAnswer();
        }
        else {
            wrongAnswer();
